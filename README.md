...
1. Using Single Camera by translation of 10cm horizontaly
...
For dual camera setup, we have captured 150 images from left and right camera which are stored in the folder named as "left" and "right" folder. Stereo images used in this approach are placed outside with names "left.png" and "right.png. Both cameras had reprojection error of 0.04 pixels. Run Dual_Camera.ipynb file to see the results obtained fron this approach. Stereo calibration results are stored in the file stereo_calibration.npz file.

Furthermore we have created the GUI for the dual camera stereo system having the following flow: 

1. Select the folder for left and right camera calibration images and calibrate the cameras. Calibration results are shown in the log. 
2. After calibration it will ask you the select the stereo images pairs where the left and right images of the scene are uploaded.
...
6. By clicking on dense button, it will implement the SGBM algorithm on the stereo images and will display the depth map and 3d point cloud. 
